...
{
  updated_params->text = malloc(1);
  memset(updated_params->text, 0, 1);
...
void UpdateOffset(Line_Params* text_parameters) { text_parameters->offset += strlen(text_parameters->text); }
...
{
  text_parameters->text = realloc(text_parameters->text, strlen(text) + 1);
  memset(text_parameters->text, 0, strlen(text) + 1);

  strncpy(text_parameters->text, text, strlen(text));
}
